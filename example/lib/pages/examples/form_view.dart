...
class Notifier extends ChangeNotifier {
  ...
  // validation error example - in case the error is from backend
  Map<String, dynamic> errors = {
    'name': ['Name is required', 'Name must be at least 3 characters long'],
    'phone': ['Phone number is required']
  };

  // apply error to the form based on the errors map
  void applyErrors() {
    forms.setMessage(errors, mergeMessages: true);
  }
}
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            ...
            IconButton(
                onPressed: () {
                  notifier.applyErrors();
                },
                icon: Icon(Hi.rssError)),
          ...
          children: [
            ...
            LzForm.select(
                ...
                onChange: (value) {
                  ...
                  final data = cities.where((e) => e['province_id'] == forms.extra('province')).toList();
            ...
            LzForm.select(
                ...
                onTap: () async {
                  final data = await getCities().overlay();
                  forms.set('city').options(data.labelValue('name', 'id'));
                }),
