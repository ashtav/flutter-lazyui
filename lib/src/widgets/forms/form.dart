...
class FormManager {
  ...
  void setMessage(Map<String, dynamic> messages, {bool mergeMessages = false}) {
    final notifiers = Map.fromIterables(models.keys, models.values.map((e) {
      e.notifier.rules = [];
      e.notifier.feedback = FormFeedback.text;
      e.notifier.groupNotifier?.clear();
    ...
    for (var entry in messages.entries) {
      ...
      if (mergeMessages && messages.length > 1) {
        if (messages.length == 2) {
          message = messages.join(' & ').toLowerCase().ucfirst;
        } else {
          final last = messages.last;
          final rest = messages.sublist(0, messages.length - 1);
          message = '${rest.join(', ')}, & $last'.toLowerCase().ucfirst;
        }
      } else {
      ...
      if (models.containsKey(key)) {
        final error = {'key': key, 'type': '', 'value': notifiers[key], 'message': message};

        final notifier = notifiers[key];

        if (notifier != null) {
          notifier.rules.add(error);
          notifier.validate();
        }
      }
    }
