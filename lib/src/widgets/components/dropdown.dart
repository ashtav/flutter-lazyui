...
/// Internal configuration class for dropdown widgets.
///
/// This class holds all the necessary data to display and manage a dropdown.
class _DropConfig {
  /// The [BuildContext] in which the dropdown is displayed.
  ...
  /// The position of the dropdown relative to the triggering widget.
  final DropPosition? position;

  /// The alignment of the dropdown.
  final DropAlign? align;

  /// The spacing between the dropdown and its triggering widget.
  final Offset? space;

  /// The child widget to display inside the dropdown.
  final Widget child;

  /// A custom builder function to build dropdown content.
  final Widget Function(List<DropOption> options)? dropBuilder;

  /// The width of the dropdown.
  final double? width;

  /// Constructor to initialize a [_DropConfig] instance.
  _DropConfig(
    this.context,
    this.key,
    this.options,
...
late _DropConfig _config;

/// Controller to manage dropdown behavior.
///
/// Use this controller to display the dropdown and manage its state.
class DropController {
  /// Displays the dropdown using the configuration stored in [_config].
  Future<DropValue?> show() async {
    return _config.context.dropdown(
      _config.key,
      options: _config.options,
      onSelect: _config.onSelect,
      space: _config.space,
      position: _config.position,
      align: _config.align,
      overlay: _config.child,
      dropBuilder: _config.dropBuilder,
      width: _config.width,
    );
  }
}
...
class LzDropdown extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    _config = _DropConfig(context, key, options, onSelect, position, align, space, childOverlay, dropBuilder, width);

    // Build the child widget using the provided builder.
    Widget child = builder(key, DropController());
